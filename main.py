...
class RobloxClient:
    ...
    def _request_launch_ticket(self, session, place_id, max_attempts=5):
        last_status = None
        ...
        for attempt in range(max_attempts):
            ...
            if ticket_resp.status_code == 403:
                renewed_csrf = ticket_resp.headers.get("x-csrf-token")
                if renewed_csrf:
                    headers["x-csrf-token"] = renewed_csrf
                    ticket_retry = session.post("https://auth.roblox.com/v1/authentication-ticket", headers=headers, timeout=10)
                    ticket = ticket_retry.headers.get("rbx-authentication-ticket")
                    if ticket:
                        return ticket, None
